<!--2.0 menu del administrador -->
<nav class="black">
	<a href="" data-activates="menu" class="button-collpase"><i class="material-icons">menu</i></a>
</nav>

<ul id="menu" class="side-nav fixed"> 
	<li>
		<div class="userView">
			<div class="background">
				<img src="../img/fondo.jpg">
			</div> 
			<a href="" ><img src="../img/perfil.jpg" class="circle" alt=""></a>
			<!--aqui actualizamos los datos del usuario admin -->
			<a href="" class="white-text">Jorge ivan</a> <br> <!--aqui nos traemos los datos de la bd del usuario -->
			<a href="" class="white-text">correo@correo</a>
		</div>
	</li>
	<li><a href=""><i class="material-icons">home</i>Inicio</a></li>	
	<li><div class="divider"></div></li>
		
</ul>